...
class SyncRekapUseCase @Inject constructor(
    private val repository: PemilihanRepository,
    private val dataStoreDiv: DataStoreDiv
) {
    operator fun invoke(syncRekap: SyncRekap): Flow<Resource<CommonStatus>> = flow {
        try {
            ...
            val request = syncRekap
            val result = repository.syncRekap(token, request)
            ...
            emit(Resource.Error(e.localizedMessage ?: "Terjadi kesalahan saat sinkronisasi rekap"))
        }
    }
}
