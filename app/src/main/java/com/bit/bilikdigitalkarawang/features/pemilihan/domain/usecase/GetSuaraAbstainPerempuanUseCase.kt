...
class GetSuaraAbstainPerempuanUseCase @Inject constructor(
    private val repository: PemilihanRepository
) {
    operator fun invoke(): Flow<Resource<Int>> = flow {
        emit(Resource.Loading())
        repository.getJumlahAbstainPerempuan()
            .map { jumlah ->
                Resource.Success(jumlah)
            }
            .catch { e ->
                emit(Resource.Error(e.localizedMessage ?: "Gagal mengambil data suara abstain"))
            }
            .collect { result ->
                emit(result)
            }
